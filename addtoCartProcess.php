...
if ($num > 0) {
    ...
    $rs2 = Database::search("SELECT * FROM `cart` WHERE `user_id` = '" . $user["user_id"] . "' AND `stock_stock_id` = '" . $stockId . "'");
    $num2 = $rs2->num_rows;
    ...
    if ($num2 > 0) {
        ...
        if ($stockQty >= $qty) {
            // Insert query
            Database::iud("INSERT INTO `cart` (`cart_qty`, `user_id`, `stock_stock_id`) VALUES ('" . $qty . "', '" . $user["user_id"] . "', '" . $stockId . "')");
