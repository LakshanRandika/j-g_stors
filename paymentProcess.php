...
if (isset($_POST["cart"]) && $_POST["cart"] == "true") {
    ...
    $rs = Database::search("SELECT * FROM `cart` WHERE `user_id`='".$user["user_id"]."'");
    $num = $rs->num_rows;
...
$merchantId = "1227049";
$merchantSecret = "MjQ3MTU4NzgzNjQ3NDA4NDIyODIxNjU4MTYxNTAyOTk3NDA1NTQ3";
$items = "";
$netTotal = 0;
$currency = "LKR";
$orderId = uniqid();
...
for ($i=0; $i < sizeof($stockList); $i++) { 
    ...
    $rs2 = Database::search("SELECT * FROM `stock` INNER JOIN `product` ON `stock`.`product_id`=`product`.`product_id` 
